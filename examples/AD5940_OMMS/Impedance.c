...
int32_t AppIMPDataProcess(int32_t * const pData, uint32_t *pDataCount)
{
  ...
  fImpCar_Type * const pOut = (fImpCar_Type*)pData; /* Create pOut with buffer address, recast */
  iImpCar_Type * pSrcData = (iImpCar_Type*)pData;  /* Copy pointer to data. New pointer pSrcData defined. */
  ...
  *pDataCount = 0;
  ...
  {
    iImpCar_Type *pDftRcal, *pDftRz; /* Declare pointers of iImpCar_Type type */
		
    pDftRcal = pSrcData++; /* Increment pointer to data */
    pDftRz = pSrcData++;
    float RzMag,RzPhase;
		float RzReal,RzImage;
    float RcalMag, RcalPhase;
    
		RzReal = (float)pDftRz->Real; /* Get member */
		RzImage = (float)pDftRz->Image;
    ...
    RzPhase = RcalPhase - RzPhase;
		RzReal = cos(RzPhase)*RzMag;
		RzImage = -sin(RzPhase)*RzMag;
    printf("Rz - Mag:%f, Phase:%f", RzMag, RzPhase);
		    
    pOut[i].Real = RzReal;
    pOut[i].Image = RzImage;
  }
